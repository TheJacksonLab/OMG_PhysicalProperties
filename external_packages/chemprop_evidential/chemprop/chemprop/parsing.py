...
def add_train_args(parser: ArgumentParser):
    ...
    parser.add_argument('--torch_seed', type=int, default=0,
                        help='Random seed to use initialize torch NN.')
...
def parse_predict_args_script(input_args) -> Namespace:
    """
    Parses arguments for prediction
    This function was modified to take args (list)
...
def parse_train_args_script(input_args) -> Namespace:
    """
    Parses arguments for training (includes modifying/validating arguments).
    This function was modified to take args (list)
    ...
    parser = ArgumentParser()
    add_train_args(parser)
    temp_args, unk_args  = parser.parse_known_args(args=input_args)
    if temp_args.atomistic:
        add_atomistic_args(parser)
    args = parser.parse_args(args=input_args)
    modify_train_args(args)
