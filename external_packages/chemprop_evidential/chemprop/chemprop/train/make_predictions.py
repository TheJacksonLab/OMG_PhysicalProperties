...
def make_predictions_original(args: Namespace, smiles: List[str] = None) -> List[Optional[List[float]]]:
    """
    ...
    with open(args.preds_path, 'w') as f:
        ...
        if orig_args.confidence:
            ...
            uncertainty_header = ["uncertainty_" + task for task in args.task_names]
            header.extend(uncertainty_header)
            std_header = ["std_" + task for task in args.task_names]
...
def make_predictions(args: Namespace, smiles: List[str] = None):
    """
    This was modified to output predictions fast

    Makes predictions. If smiles is provided, makes predictions on smiles. Otherwise makes predictions on args.test_data.
    :param args: Arguments.
    :param smiles: Smiles to make predictions on.
    :return: df_return, df_std, scaler (used to normalize for multi-obejctive optimization)
    """
    if args.gpu is not None:
        torch.cuda.set_device(args.gpu)
    ...
    # return predictions
    # 1) SMILES df
    df_smiles = pd.DataFrame(columns=['smi'], data=test_smiles)

    # 2) prediction
    avg_preds_col = get_task_names(args.data_path)
    avg_preds_arr = np.array(avg_preds)
    df_pred = pd.DataFrame(columns=avg_preds_col, data=avg_preds_arr)

    # 3) true values
    true_target_col = ["true_" + task for task in args.task_names]
    true_target_arr = np.array(test_data.targets())
    df_true = pd.DataFrame(columns=true_target_col, data=true_target_arr)

    # 4) uncertainty
    uncertainty_col = ["uncertainty_" + task for task in args.task_names]
    uncertainty_arr = np.array(confidence)
    df_uncertainty = pd.DataFrame(columns=uncertainty_col, data=uncertainty_arr)

    # 5) std
    std_col = ["std_" + task for task in args.task_names]
    std_arr = np.array(std)
    df_std = pd.DataFrame(columns=std_col, data=std_arr)

    # concat
    df_return = pd.concat([df_smiles, df_pred, df_true, df_uncertainty, df_std], axis=1)

    return df_return, df_std, scaler


def make_uncertainty_predictions_without_true_values(args: Namespace, smiles: List[str] = None):
    """
    This was modified to output predictions fast. This function is different from 'make_prediction' so that
    thus also can work when true target values are not provided.
    ...
    # concat
    df_return = pd.concat([df_smiles, df_pred, df_uncertainty, df_std], axis=1)
