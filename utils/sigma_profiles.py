...
def get_seg_DataFrame(COSMO_contents):
    ...
    if "DMol3/COSMO Results" in COSMO_contents:
        ...
        table_assign = ['n', 'atom', 'x / a.u.', 'y / a.u.', 'z / a.u.', 'charge / e', 'area / A^2',
                        'charge/area / e/A^2', 'potential']
    elif "Gaussian COSMO output" in COSMO_contents:
        # Gaussian09: same unit (Bohr in G09) as Dmol3 but format is different
        sdata = re.search(r"\(X, Y, Z\)[\sa-zA-Z0-9\[\]/\#\n]+\#\n([\s\S]+)", COSMO_contents, re.DOTALL).group(
            1).rstrip()
        table_assign = ['n', 'atom', 'x / a.u.', 'y / a.u.', 'z / a.u.', 'charge / e', 'area / A^2',
                        'charge/area / e/A^2', 'potential']
    elif "GAMESS/COSab RESULTS" in COSMO_contents:
        # GAMESS: same unit (Bohr in GAMESS/COSab) as Dmol3 but format is different
        sdata = re.search(r"\(X, Y, Z\)[\sa-zA-Z0-9\(\)\./\*]+\n([\s0-9\-\n.]+)(=+)", COSMO_contents, re.DOTALL).group(
            1).rstrip()
        table_assign = ['n', 'atom', 'x / a.u.', 'y / a.u.', 'z / a.u.', 'charge / e', 'area / A^2',
                        'charge/area / e/A^2', 'potential']
    else:  # ORCA
        # + should be inserted to repeat finding symbols in []
        # (...) should be inserted to specify the group
        sdata = re.search(r'SURFACE POINTS \(A.U.\)[\s(a-zA-Z0-9-)_#]+\n+([\s\S]+)', COSMO_contents,
                          re.DOTALL).group(1).rstrip()  # Switch_F
        table_assign = ['x / a.u.', 'y / a.u.', 'z / a.u.', 'area / a.u.', 'potential', 'charge / e', 'w_leb',
                        'Switch_F', 'G_width', 'atom']

        # sdata = re.search(r'SURFACE POINTS \(A.U.\)[\s]+\(Hint - charge NOT scaled by FEps\)+[\n]+#--------------------'
        #                   r'----------------------------------------+[\sA-Z]'
        #                   , COSMO_contents, re.DOTALL).group(0)
        # sdata = re.search('SURFACE POINTS \(A.U.\)[\s]+\(Hint - charge NOT scaled by FEps\)', COSMO_contents, re.DOTALL).group()
        # print(COSMO_contents)
        # exit()
        # sdata = re.search(r'SURFACE POINTS \(A.U.\)[\s(a-zA-Z0-9-)#]+[\s0-9.-]+', COSMO_contents, re.DOTALL).group(0)
        # sdata = re.search(r'SURFACE POINTS \(A.U.\)[\s(a-zA-Z0-9-)#]+\n+([\s\S]+)', COSMO_contents, re.DOTALL).group(1).rstrip()
        # # sdata = re.search('SURFACE POINTS \(A.U.\)[\s]+\(+[\sa-zA-Z0-9-]+', COSMO_contents).group()
        # print(sdata)
        # # print(sdata[-1])
        # exit()
        # sdata = re.search(r"\(SURFACE POINTS \(A.U.\)[\sa-zA-Z0-9(\-)\#]", COSMO_contents, re.DOTALL).group(1).rstrip()
...
def get_atom_DataFrame(COSMO_contents):
    ...
    if "DMol3/COSMO Results" in COSMO_contents:
        ...
        # + should be inserted to repeat finding symbols in []
        # (...) should be inserted to specify the group
        sdata = re.search(r'CARTESIAN COORDINATES \(A.U.\)[\s\w+(.)-]+[#-]+\n+([\s0-9-.]+)+#', COSMO_contents,
                          re.DOTALL).group(1).rstrip()
...
def get_area_volume(COSMO_contents):
    ...
    if "DMol3/COSMO Results" in COSMO_contents:
        ...
        volume = float(
            re.search(r"Total volume of cavity \(A\*\*3\)           =(.+)\n", COSMO_contents).group(1).strip())
    elif "Gaussian COSMO output" in COSMO_contents:
        ...
        re_search = re.search(r'xi_gen\n\n([\s0-9.]+)+# Volume+\n([\s0-9.]+)+# Area', COSMO_contents)
        volume = float(re_search.group(1))  # a.u.
        area = float(re_search.group(2))  # a.u.
...
class ORCACOSMOParser(object):
    ...
    def __init__(self, inpath, geometry_path, max_sigma, num_profiles=3, averaging=None):
        ...
        geometry_contents = open(geometry_path).readlines()
        geometry_contents = geometry_contents[2:]  # exclude the first two lines
        atom_list = list()
        for line in geometry_contents:
            tmp_line = line.split()
            atom_list.append(tmp_line[0])
        self.df_atom['atom'] = atom_list
...
# checked
def get_sigma_profiles(inpath, outpath, geometry_path, num_profiles=1, averaging='Mullins', max_sigma=0.025):
    """
    This function generates & saves a sigma profile from a COSMO file to the output path.
    :param inpath: the path to the cosmo file (.cpcm) that is to be processed
    :param outpath: the path to the sigma profile file (.sigma) that is to be generated
    :param geometry_path: the path to the geometry (.xyz) used for the COSMO calculation.
    :param num_profiles: the number of profiles to generate, either 1 or 3.
    :param averaging: the scheme used to do averaging of profiles.
    :param sigma_range: the max simga value to cut the sigma profiles. This should be 0.025 or 0.050
    Either 'Mullins' to use f_decay = 1 and r_av = 0.8176300195 A or 'Hsieh' to use f_decay = 3.57 and r_av = sqrt(7.25/pi)")
    :return: None
    """
    orca = read_orca(inpath=inpath, num_profiles=num_profiles, averaging=averaging, geometry_path=geometry_path,
                     max_sigma=max_sigma)
